...
fun formatHoverItems(stack: ItemStack, knownItems: MutableList<ItemStack>, player: Player?): MessageEmbed? {
    ...
    val tooltip = stack.getTooltipLines(Item.TooltipContext.of(player?.level()), player, TooltipFlag.NORMAL).map(::formatComponentToMarkdown)
    return EmbedBuilder()
        .setTitle("${tooltip[0]} ${if (stack.count > 1) "(${stack.count})" else ""}")
        .setDescription(tooltip.drop(1).let {
            if (stack.has(DataComponents.CUSTOM_NAME)) {
                listOf(stack.get(DataComponents.CUSTOM_NAME)).plus(it)
            } else it
        }.joinToString("\n"))
        .let { builder: EmbedBuilder ->
